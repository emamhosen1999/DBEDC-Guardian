...
class StoreDailyWorkRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        ...
        return [
            ...
            'time.regex' => 'RFI Time must be in HH:MM format (24-hour).',
    ...
    private function getCompletionTimeValidation(): array
    {
        ...
        if ($status === DailyWork::STATUS_COMPLETED) {
            return ['required', 'string', 'regex:/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/', 'after:time'];
        }
    ...
    private function validateCrossFieldRules($validator): void
    {
        ...
        // Validate time logic
        if (!empty($data['time']) && !empty($data['completion_time'])) {
            $plannedTime = strtotime($data['time']);
