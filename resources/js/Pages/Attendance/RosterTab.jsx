...
export default function RosterTab({ month, departments = [], isActive = true }) {
    ...
    const handleCellClick = (userId, date, cell) => {
        // Toggle: assigned shift -> cleared; empty/off -> no-op (shift picking is out of scope for this pass).
        if (cell && !cell.off) {
            updateCell.mutate({ userId, date, shiftId: null });
        }
    };
