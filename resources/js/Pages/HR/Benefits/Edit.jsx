...
export default function BenefitsEdit({ benefit }) {
    const { data, setData, put, processing, errors } = useForm({
        name: benefit.name || '',
        type: benefit.type || '',
        description: benefit.description || '',
        company_contribution_rate: benefit.company_contribution_rate || '',
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('hr.benefits.update', benefit.id));
    };
    ...
    return (
        <App>
            ...
            <div className="mx-auto w-full max-w-4xl space-y-6 p-4 sm:p-6">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900">Edit Benefit</h1>
                    <p className="text-sm text-gray-600">Update benefit plan details.</p>
                </div>
                ...
                <form onSubmit={submit} className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 sm:p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Link href={route('hr.benefits.show', benefit.id)} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Cancel
                        </Link>
                    </div>
                </form>
