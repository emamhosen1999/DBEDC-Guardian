...
const MemberTable = ({ 
  ...
  totalUsers = 0,
  loading = false,
  updateMemberOptimized,
  deleteMemberOptimized
}) => {
  ...
  const [employeeToDelete, setMemberToDelete] = useState(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        if (response.status === 200) {
          // Update optimistically
          if (deleteMemberOptimized) {
            deleteMemberOptimized(employeeToDelete.id);
          }
        ...
        if (error.response?.status === 403) {
          ...
          errorMessage = 'Member not found or already deleted';
  ...
  const handleImageUpdate = (employeeId, newImageUrl) => {
    ...
    if (updateMemberOptimized) {
      updateMemberOptimized(employeeId, {
  ...
  const getEligibleManagers = useCallback((user, currentManagerId = null) => {
    ...
    const sameDeptWorkForce = allManagers.filter(m => 
      String(m.department_id) === String(userDepartmentId) && m.id !== user.id
    );
    const isDepartmentHead = !sameDeptWorkForce.some(m => 
