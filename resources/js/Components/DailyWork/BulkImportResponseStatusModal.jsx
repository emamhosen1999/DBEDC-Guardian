...
const BulkImportResponseStatusModal = ({
    ...
    const getStatusBadge = (status) => {
        ...
        const colorMap = { success: 'green', danger: 'red', warning: 'amber', primary: 'indigo', secondary: 'violet', default: 'gray' };
        return <Badge size="1" color={colorMap[config.color]||'gray'} variant="soft">{config.label}</Badge>;
    };
                ...
                <>
                        <Dialog.Title>
                            <Flex align="center" gap="2" wrap="wrap">
                                ...
                                <Text weight="bold" size={{ initial: '2', sm: '3' }}>Import RFI Response Status</Text>
                            </Flex>
                        </Dialog.Title>
                        <Dialog.Description size="2" color="gray" mb="3">Upload an Excel file with RFI numbers, response status, and date</Dialog.Description>
                        ...
                        <Box style={{ overflowY: 'auto', maxHeight: 'min(65vh, 500px)' }} py="2">
                                            ...
                                            <table style={{ fontSize: 12, width: '100%' }}>
                                                <thead>
                                                    <tr style={{ borderBottom: '1px solid var(--gray-a4)' }}>
                                                        ...
                                                        <th style={{ textAlign: 'left', padding: '8px', fontWeight: 600 }}>Response Status</th>
                                                        <th style={{ textAlign: 'left', padding: '8px', fontWeight: 600 }}>Response Date</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr style={{ borderBottom: '1px solid var(--gray-a3)' }}>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>RFI-001</td>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>approved</td>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>2024-12-20</td>
                                                    </tr>
                                                    <tr>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>RFI-002</td>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>rejected</td>
                                                        <td style={{ padding: '8px', color: 'var(--gray-11)' }}>2024-12-21</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </ScrollArea>
                                        <Text size="1" style={{ color: 'var(--gray-11)', marginTop: 8 }}>
                                            Valid statuses: <Text style={{ fontFamily: 'monospace' }}>approved, rejected, returned, concurred, not_concurred</Text>
                                        </Text>
                                    </Box>
                                </Flex>
                            )}
                                            ...
                                            <ScrollArea style={{ maxHeight: 192 }}>
                                                <Flex direction="column" gap="1" pr="2">
                                                    {validationErrors.errors.map((error, idx) => (
                                                        <Box key={idx} p="2" style={{ borderRadius: 'var(--radius-1)', background: 'var(--red-a3)', border: '1px solid var(--red-a6)' }}>
                            ...
                            {step === 'objection-decision' && (
                                <Flex direction="column" gap="4">
                                        ...
                                        <ScrollArea style={{ maxHeight: 128 }}>
                                            <Flex direction="column" gap="1" pr="2">
                                                {objectedWorks.map((work, idx) => (
                                                    <Flex key={work.id || idx} justify="between" align="center" p="2" style={{ borderRadius: 'var(--radius-1)', background: 'var(--amber-a2)', border: '1px solid var(--amber-a6)' }}>
                                                        <Flex align="center" gap="2">
                                                            <ExclamationTriangleIcon style={{ width: 16, height: 16, flexShrink: 0, color: 'var(--amber-9)' }} />
                                                            <Text size="2" weight="medium">{work.number}</Text>
                                    ...
                                    <Box>
                                        <Text size="1" color="gray" as="p" mb="1">Override Reason (required to update objected RFIs)</Text>
                            ...
                            {step === 'result' && result && (
                                <Flex direction="column" gap="4">
                                    ...
                                    <Flex direction="column" align="center" py="4">
                                        <CheckCircledIcon style={{ width: 64, height: 64, color: 'var(--green-9)', margin: '0 auto 12px' }} />
                                        <Text size="6" weight="bold" style={{ color: 'var(--green-11)' }}>
                                            Import Complete
                                        </Text>
                                        <Text style={{ color: 'var(--gray-11)', marginTop: 4 }}>{result.message}</Text>
                                    </Flex>
                                    ...
                                    {result.updated?.length > 0 && (
                                        <Box>
                                            ...
                                            <ScrollArea style={{ maxHeight: 96 }}>
                                                <Flex wrap="wrap" gap="1" pr="2">{result.updated.map((work) => <Badge key={work.id} size="1" color="green" variant="soft">{work.number}</Badge>)}</Flex>
                                            </ScrollArea>
                                        </Box>
                                    )}
                                    ...
                                    {result.skipped?.length > 0 && (
                                        <Box>
                                            <Flex align="center" justify="between" mb="2">
                                                ...
                                                <Button size="1" variant="soft" color="amber" onClick={handleDownloadSkipped}><DownloadIcon style={{ width: 12, height: 12 }} /> Download</Button>
                                            </Flex>
                                            <ScrollArea style={{ maxHeight: 96 }}>
                                                <Flex wrap="wrap" gap="1" pr="2">{result.skipped.map((work) => <Badge key={work.id} size="1" color="amber" variant="soft">{work.number}</Badge>)}</Flex>
                                            </ScrollArea>
                                        </Box>
                                    )}
                                    ...
                                    {result.not_found?.length > 0 && (
                                        <Box>
                                            ...
                                            <ScrollArea style={{ maxHeight: 96 }}><Flex wrap="wrap" gap="1" pr="2">{result.not_found.map((item, idx) => <Badge key={idx} size="1" variant="soft" color="gray">{item.rfi_number}</Badge>)}</Flex></ScrollArea>
                                        </Box>
                                    )}
                                </Flex>
                            )}
                        ...
                        <Flex gap="2" pt="3" style={{ borderTop: '1px solid var(--gray-a4)', flexDirection: 'column-reverse' }}>
                            {step === 'upload' && (<>
                                ...
                                <Button color="indigo" onClick={() => handleUpload(false, false)} loading={loading} disabled={!file} style={{ width: '100%' }}>
                                    {!loading && <ClipboardIcon style={{ width: 16, height: 16 }} />} Import & Update
                                </Button>
                            </>)}
                            {step === 'validation-errors' && <Button variant="ghost" color="gray" onClick={handleClose} style={{ width: '100%' }}>Close</Button>}
