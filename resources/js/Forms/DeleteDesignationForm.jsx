    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                showToast.success(response.data.message || 'Designation deleted successfully');
                if (onSuccess) onSuccess(designation, 'delete');
                onClose();
            }
        } catch (error) {
            if (error.response?.status === 400 || error.response?.status === 422) {
                ...
                showToast.error('Designation not found or already deleted');
            } else {
            ...
            <Dialog.Content style={{ maxWidth: 450 }}>
                ...
                <Flex justify="end" gap="3">
                    ...
                    <Button color="red" onClick={handleDelete} disabled={loading || hasEmployees}>
                        {loading ? <Spinner size="1" /> : 'Delete Designation'}
                    </Button>
                </Flex>
            </Dialog.Content>
        </Dialog.Root>
